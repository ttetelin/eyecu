...
int main()
{
	...
	volatile int rd,wr; // read character cound and write character count

	fd=open("/dev/ttyO1",(O_RDWR | O_NOCITY | O_NDELAY)); // open the serial port file
	...
	wr = write(fd,"ATZ\r",4); // write(filehandle, character array, bytecount)
